#ifndef AlgheLib_cppex_util_list_h__
#define AlgheLib_cppex_util_list_h__
	...
	namespace util {
		...
		***/
		...
		public:
			...
			// Extended methods. Carefull they are O(n)
			size_t Length() const;
			T* FindAt(size_t index);
			const T* FindAt(size_t index) const;
		...
		private:
			// Hide copy-constructor and assignment operator
			TList (const TList &) {};
			TList & operator= (const TList &) { return *this; };
		};
		...
		//=============================================================================
		template<class T>
		const T* TList<T>::FindAt(size_t index) const {
			size_t n = 0;
			const T* p = Head();
			while(p != NULL && n != index) {
				n++;
...
#define LIST_DECLARE(T, link) cppex::util::TListDeclare<T, offsetof(T, link)>

// Define a field within a structure that will be used to link it into a list
#define LIST_LINK(T) cppex::util::TLink<T>

// Define a pointer to a list
#define LIST_PTR(T) cppex::util::TList<T> *
